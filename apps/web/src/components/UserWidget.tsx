...
import { User, Shield, Users, LogOut, ChevronRight, X } from 'lucide-react';
import { getCurrentUser, logout } from '@/lib/auth'; // Assuming logout exists or we clear cookies
                ...
                <div className="fixed inset-y-0 right-0 w-80 bg-background border-l border-border shadow-2xl z-[60] p-6 animate-in slide-in-from-right">
                    ...
                    <div className="space-y-4">
                        <div className="p-3 bg-muted/50 rounded-lg">
                            ...
                            <p className="font-medium">{user.organizationName || 'My Organization'}</p>
                        </div>

                        <div className="space-y-3">
                            <TeamMember name={user.name} email={user.email} role={user.role} isMe />
                            {/* Mock Data for Demo */}
                            <TeamMember name="Sarah Engineer" email="sarah@company.com" role="Admin" status="online" />
                            <TeamMember name="Mike DevOps" email="mike@company.com" role="Member" status="offline" />
                            <TeamMember name="Alex Product" email="alex@company.com" role="Viewer" status="away" />
                        </div>
                    </div>
                </div>
            )}
