...
const logger = new utils_1.Logger('MongoDBAdapter');
/**
 ...
 */
class MongoDBAdapter extends IDBAdapter_1.BaseDBAdapter {
    constructor() {
        super(...arguments);
    ...
    async executeQuery(request) {
        ...
        try {
            ...
            if (queryObj.find) {
                ...
                const options = {};
                if (queryObj.sort)
                    options.sort = queryObj.sort;
                if (queryObj.limit)
                    options.limit = queryObj.limit;
                if (queryObj.skip)
                    options.skip = queryObj.skip;
