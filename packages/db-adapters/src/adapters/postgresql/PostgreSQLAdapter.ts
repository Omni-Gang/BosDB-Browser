 ...
 */
export class PostgreSQLAdapter extends BaseDBAdapter {
    ...
    `;
        ...
        const pool = poolInfo?.pool || new Pool();
    ...
    async listTables(connectionId: string, schema: string = 'public'): Promise<Table[]> {
        const query = `
      SELECT 
